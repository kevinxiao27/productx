import { mildKeywords } from "./mildKeywords.js";
import { severeKeywords } from "./severeKeywords.js";
...
export const mapSentimentToDanger = (sentimentResults) => {
  ...
  for (const result of sentimentResults) {
    ...
    if (sentiment === "NEGATIVE" && confidence > 0.8 && (mildRegex.test(cleanedText) || severeRegex.test(cleanedText))) {
      return "danger";
    }

    // 3. Low-medium confidence + severe = danger
    if (sentiment === "NEGATIVE" && confidence > 0.5 && severeRegex.test(cleanedText)) {
      return "danger";
    }

    // 4. Neutral + severe keyword = danger
    if (sentiment === "NEUTRAL" && confidence > 0.3 && confidence < 0.5 && severeRegex.test(cleanedText)) {
      return "danger";
    }
